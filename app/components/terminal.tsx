...
export default function TComponent({slug}: any) {
    ...
    useEffect(() => {
      const initializeInstance = async () => {
        if (createParam) {
          setIsLoading(true)
          setHistory([
            { command: "", output: "Creating new BreadKV instance, usually takes 15-35 seconds. Please wait..." }
          ])
          ...
          try {
            ...
            setHistory([
              { command: "", output: "Instance created successfully! Welcome to your BreadKV instance. You will have 15 minutes since this instance's creation to use it, after which it will be destroyed. Type 'help' for available commands." }
            ])
          } catch (err) {
          } finally {
            setIsLoading(false)
    ...
    const [history, setHistory] = useState<{ command: string; output: string }[]>([
        { command: "", output: "Welcome to your BreadKV instance. Type 'help' for available commands." },
    ])
          ...
          >
            ...
            {!isLoading && history.map((item, index) => (
              <div key={index}>
                ...
                {item.output && <div className="whitespace-pre-wrap mb-2 pl-4">{item.output}</div>}
              </div>
            ))}
